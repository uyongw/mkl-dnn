...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
    typename utils::enable_if<
        (fmt_i == oihw && fmt_o == Ihw16io)
        || (fmt_i == goihw && fmt_o == gIhw16io)
    >::type>
{
    static bool is_applicable(const memory_desc_wrapper &input_d,
            const memory_desc_wrapper &output_d) {
    ...
    static status_t execute(const cpu_reorder_pd_t *pd,
        ...
        const auto &_g_oihw_d = order_keep ? input_d : output_d;
        const auto _g_oihw_st = _g_oihw_d.blocking_desc().strides[0];
        const auto &dims = input_d.dims();
...
#               pragma omp simd collapse(2)
                for (int i = 0; i < blksize; ++i) {
                    for (int o = 0; o < OC; ++o) {
                        const auto _g_oihw_off = o * _g_oihw_st[w_groups + 0]
                            + i * _g_oihw_st[w_groups + 1];
                        if (order_keep)
                            out[i * OC + o] = data_t<type_o>(inp[_g_oihw_off]);
                        else
                            out[_g_oihw_off] = data_t<type_o>(inp[i * OC + o]);
                    }
                }
            } else {
#               pragma omp simd collapse(2)
                for (int i = 0; i < blksize; ++i) {
                    for (int o = 0; o < OC; ++o) {
                        ...
                        if (order_keep)
                            out[i * OC + o] =
                                data_t<type_o>(alpha * inp[_g_oihw_off]
                                        + (beta ? beta * out[i * OC + o] : 0));
                        else
                            out[_g_oihw_off] =
                                data_t<type_o>(alpha * inp[i * OC + o]
                                        + (beta ? beta * out[_g_oihw_off] : 0));
                    }
                }
            }
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
    typename utils::enable_if<
        (fmt_i == goihw && fmt_o == gIhwo16i)
        || (fmt_i == oihw && fmt_o == Ihwo16i)
    >::type>
{
    static bool is_applicable(const memory_desc_wrapper &input_d,
            const memory_desc_wrapper &output_d) {
...
#               pragma omp simd
                for (int i = 0; i < blksize; ++i) {
                    const auto _g_oihw_off = i * _g_oihw_d.blocking_desc()
                        .strides[0][w_groups + 1];
                    if (order_keep)
                        out[i] = data_t<type_o>(inp[_g_oihw_off]);
                    else
                        out[_g_oihw_off] = data_t<type_o>(inp[i]);
                }
            } else {
#               pragma omp simd
                for (int i = 0; i < blksize; ++i) {
                    const auto _g_oihw_off = i * _g_oihw_d.blocking_desc()
                        .strides[0][w_groups + 1];
                    if (order_keep)
                        out[i] = data_t<type_o>(alpha * inp[_g_oihw_off]
                                       + (beta ? beta * out[i] : 0));
                    else
                        out[_g_oihw_off] = data_t<type_o>(alpha * inp[i]
                                       + (beta ? beta * out[_g_oihw_off] : 0));
                }
            }
        };
...
#       pragma omp parallel for collapse(5) schedule(static)
        for (int g = 0; g < _G; ++g) {
            for (int I = 0; I < dims[w_groups + 1] / blksize; ++I) {
                for (int h = 0; h < dims[w_groups + 2]; ++h) {
                    for (int w = 0; w < dims[w_groups + 3]; ++w) {
                        for (int o = 0; o < dims[w_groups + 0]; ++o) {
                            auto inp = &input[input_d.blk_off<!w_groups>(g,
                                                       o, i_mult*I, h, w)];
                            auto out = &output[output_d.blk_off<!w_groups>(g,
                                                       o, o_mult*I, h, w)];
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
    typename utils::enable_if<
        (fmt_i == Ihw16io && fmt_o == Ihwo16i)
        || (fmt_i == gIhw16io && fmt_o == gIhwo16i)
    >::type>
{
    static bool is_applicable(const memory_desc_wrapper &input_d,
            const memory_desc_wrapper &output_d) {
    ...
    static status_t execute(const cpu_reorder_pd_t *pd,
        ...
        constexpr bool w_groups = fmt_i == gIhw16io;
        const auto &dims = input_d.dims();
        const int blksize = 16;
        const int OC = dims[w_groups + 0];
        const float alpha = alpha_, beta = beta_;
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
    typename utils::enable_if<
        (fmt_i == goihw && fmt_o == gOhwi16o)
        || (fmt_i == oihw && fmt_o == Ohwi16o)
    >::type>
{
    static bool is_applicable(const memory_desc_wrapper &input_d,
            const memory_desc_wrapper &output_d) {
...
#               pragma omp simd
                for (int o = 0; o < blksize; ++o) {
                    const auto _g_oihw_off = o * _g_oihw_st[w_groups + 0];
                    if (order_keep)
                        out[o] = data_t<type_o>(inp[_g_oihw_off]);
                    else
                        out[_g_oihw_off] = data_t<type_o>(inp[o]);
                }
            } else {
#               pragma omp simd
                for (int o = 0; o < blksize; ++o) {
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
    typename utils::enable_if<
        (fmt_i == goihw && fmt_o == gOhw16oi)
        || (fmt_i == oihw && fmt_o == Ohw16oi)
    >::type>
{
    static bool is_applicable(const memory_desc_wrapper &input_d,
            const memory_desc_wrapper &output_d) {
                ...
                for (int o = 0; o < blksize; ++o) {
                    for (int i = 0; i < IC; ++i) {
                        const auto _g_oihw_off = o * _g_oihw_st[w_groups + 0]
                            + i * _g_oihw_st[w_groups + 1];
                        if (order_keep)
                            out[o * IC + i] = data_t<type_o>(inp[_g_oihw_off]);
                        else
                            out[_g_oihw_off] = data_t<type_o>(inp[o * IC + i]);
                    }
                }
            } else {
#               pragma omp simd collapse(2)
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
    typename utils::enable_if<
        (fmt_i == Ohwi16o && fmt_o == Ohw16oi)
        || (fmt_i == gOhwi16o && fmt_o == gOhw16oi)
    >::type>
{
    static bool is_applicable(const memory_desc_wrapper &input_d,
            const memory_desc_wrapper &output_d) {
    ...
    static status_t execute(const cpu_reorder_pd_t *pd,
        ...
        constexpr bool w_groups = fmt_i == gOhwi16o;
        const auto &dims = input_d.dims();
        const int blksize = 16;
        const int IC = dims[w_groups + 1];
        const float alpha = alpha_, beta = beta_;
